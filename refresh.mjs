...
/* Repair pass for the weekly series, run over the MERGED output rather than
   only over what was fetched this run.

   Why it exists: the weekly rotation refreshes about five symbols a night, so a
   series stored before the adjusted endpoint was wired up would otherwise sit
   there wrong for days waiting its turn. GOOGL read -88% over five years when
   the real figure was +138%, because a 20:1 split was still in the numbers.

   Why the threshold is 3 and not 2: on a weekly bar a genuine 50% drawdown and
   a 2:1 split are the same shape, so those stay report-only in findCliffs().
   Nothing legitimate takes a listed company down threefold in one week, so a
   step that lands near a declared 3:1 or wider is a split and safe to undo.
   On an already-adjusted series there is no such step and this is a no-op. */
const WEEKLY_SPLIT_FACTORS = [3, 4, 5, 10, 20];
const WEEKLY_MIN_RATIO = 2.6;    // just under 3:1 at the loose end of tolerance
const WEEKLY_TOLERANCE = 0.15;   // a weekly step carries a whole week of real movement
function healWeekly(series){
  const out = series.slice();
  const notes = [];
  for(let i = out.length - 1; i > 0; i--){
    const ratio = out[i-1] / out[i];
    if(!isFinite(ratio) || ratio < WEEKLY_MIN_RATIO) continue;
    let best = null, bestErr = Infinity;
    for(const f of WEEKLY_SPLIT_FACTORS){
      const err = Math.abs(ratio - f) / f;
      if(err < bestErr){ bestErr = err; best = f; }
    }
    if(best == null || bestErr > WEEKLY_TOLERANCE) continue;
    for(let k = 0; k < i; k++) out[k] /= best;
    notes.push(`${best}:1 at week ${i} (observed ${ratio.toFixed(2)}x)`);
  }
  return { series: out, notes };
}
...
async function main(){
  ...
  const quotes = {}, missing = [], healedWeek = [];
  let staleWeek = 0;
  for(const sym of symbols){
    ...
    if(d && d.length && w && w.length){
      const healed = healWeekly(w);
      if(healed.notes.length){
        log(`  ${sym} weekly de-split: ${healed.notes.join(', ')}`);
        healedWeek.push(sym);
      }
      quotes[sym] = { d, w: round(healed.series), wAt };
  ...
  if(healedWeek.length) log(`weekly de-split repaired: ${healedWeek.join(', ')}`);
